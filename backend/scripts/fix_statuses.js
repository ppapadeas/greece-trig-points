...
const ARCGIS_FIELDS = 'TR_FULL_CO,TR_YPARXI';
...
const fetchAllArcgisData = async () => {
  ...
  console.log(`- Step B: Fetching status data in pages...`);
...
const runStatusFix = async () => {
    ...
    console.log(`\n🔄 Starting status update for ${arcgisFeatures.length} records...`);
    ...
    try {
        ...
        for (const feature of arcgisFeatures) {
            ...
            // If TR_YPARXI is exactly 1, status is 'OK'. For everything else (0, null, etc.), it's 'UNKNOWN'.
            const newStatus = attr.TR_YPARXI === 1 ? 'OK' : 'UNKNOWN';

            const result = await client.query(
                `UPDATE points SET status = $1, updated_at = NOW() WHERE gys_id::integer = $2`,
                [newStatus, gysId]
            );
    ...
    console.log(`\n--- Status Fix Report ---`);
    console.log(`✅ Successfully updated the status for ${updatedCount} matching records.`);
    console.log('-------------------------\n');
};

const main = async () => {
    console.log('⚠️  This script will modify the status of points in the database.');
    await runStatusFix();
    await pool.end();
};

main().catch(err => {
    console.error('\n❌ A critical error occurred during the status fix:');
    if (err.response) {
